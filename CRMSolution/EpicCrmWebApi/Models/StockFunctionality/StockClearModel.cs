...
{
    public class StockClearModel : IValidatableObject
    {
        ...
        public IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
        {
            ...
            StockBalanceRec = balances.First();
            if (StockBalanceRec == null ||
                StockBalanceRec.CyclicCount != BalanceRecordCyclicCount ||
                StockBalanceRec.StockQuantity < StockRequestFullRec.Quantity)
            {
                yield return new ValidationResult("The Stock balance record either does not exist/has changed or does not enough quantity to fulfill the requested operation. Please refresh the page and try again.");
                yield break;
            }
