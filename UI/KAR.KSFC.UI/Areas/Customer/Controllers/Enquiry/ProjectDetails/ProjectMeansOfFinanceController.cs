...
namespace KAR.KSFC.UI.Areas.Customer.Controllers.Enquiry.ProjectDetails
{
    ...
    {
        private const string resultViewPath = "~/Areas/Customer/Views/Enquiry/ProjectDetails/ProjectMeansOfFinance/";
        private const string viewPath = "../../Areas/Customer/Views/Enquiry/ProjectDetails/ProjectMeansOfFinance/";
        ...
        public async Task<IActionResult> CreateOrEdit(int id = 0)
        {
            try
            {
                ...
                else
                {
                    var projMOFList = _sessionManager.GetProjectMeansOfFinanceList();
                    ProjectMeansOfFinanceDTO pro = projMOFList.Where(x => x.EnqPjmfId == id).FirstOrDefault();
                    var ddl = await _enquirySubmissionService.PopulateFinanceTypeList((int)pro.PjmfCd);
                    ViewBag.ListProjectFinanceType = ddl;
                    _logger.Information("Completed - CreateOrEdit method for " + id);
        ...
        public async Task<IActionResult> CreateOrEdit(int id, ProjectMeansOfFinanceDTO pro)
        {
            try
            {
                _logger.Information(string.Format("Started - CreateOrEdit method for Id: {0} EnqPjmfId :{1} EnqtempId :{2} PjmfCd :{3} EnqPjmfValue :{4} MfcatCd :{5} UniqueId",
                    id, pro.EnqPjmfId, pro.EnqtempId, pro.PjmfCd, pro.EnqPjmfValue, pro.MfcatCd, pro.UniqueId));
                ...
                if (ModelState.IsValid)
                {
                    ...
                    {
                        if (item.MfcatCdNavigation.PjmfDets.ToLower() == "equity")
                            totalEquity += Convert.ToDecimal(item.EnqPjmfValue);
                        if (item.MfcatCdNavigation.PjmfDets.ToLower() == "debt")
                            totalDedt += Convert.ToDecimal(item.EnqPjmfValue);
                    }
                    ViewBag.TotalEquity = totalEquity;
                    ViewBag.TotalDebt = totalDedt;
                    _logger.Information(string.Format("Completed - CreateOrEdit method for Id: {0} EnqPjmfId :{1} EnqtempId :{2} PjmfCd :{3} EnqPjmfValue :{4} MfcatCd :{5} UniqueId",
                    id, pro.EnqPjmfId, pro.EnqtempId, pro.PjmfCd, pro.EnqPjmfValue, pro.MfcatCd, pro.UniqueId));
        ...
        public ActionResult Delete(int id)
        {
            ...
            {
                _logger.Error("Error occured while loading Delete method. Error message is: " + ex.Message + Environment.NewLine + "The stack trace is: " + ex.StackTrace);
                return View("~/Views/Shared/Error.cshtml");
            }
        }
    }
