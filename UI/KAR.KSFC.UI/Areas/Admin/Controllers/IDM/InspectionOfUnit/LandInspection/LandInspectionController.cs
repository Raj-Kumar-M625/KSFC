...
namespace KAR.KSFC.UI.Areas.Admin.Controllers.IDM.InspectionOfUnit.LandInspection
{
    [Area(Constants.Admin)]
    [Authorize(Roles = RolesEnum.Employee)]

    public class LandInspectionController : Controller
    {
        private readonly ILogger _logger;
        private readonly SessionManager _sessionManager;
      

        public LandInspectionController( ILogger logger, SessionManager sessionManager)
        {
            _logger = logger;
        ...
        //Updated By Gagana - Added LandType dropdown
        [HttpGet]
        public IActionResult Create(long accountNumber,long InspectionId,byte OffcCd,string LoanSub, bool firstinspection)
        {
            try
            {
                ...
                ViewBag.firstbuildinginspection = firstinspection;
                ...
                return View(Constants.LandresultViewPath + Constants.createCS);
        ...
        public IActionResult Create(IdmDchgLandDetDTO landInspection)
        {
            try
            {
                ...
                var number = Convert.ToInt64(rand.Next(maxValue + 1).ToString("D4"));
                    ...
                    return Json(new { isValid = true, data = list.LoanAcc, html = Helper.RenderRazorViewToString(this, Constants.LandviewPath + Constants.ViewAll, landInspectionDetails) });
                
                //return Json(new { isValid = false, html = Helper.RenderRazorViewToString(this, Constants.LandviewPath + Constants.Create, landInspection) });
            }
            catch (System.Exception ex)
            {
        ...
        public IActionResult ViewRecord(string id = "")
        {
            try
            {
                ...
                var AllLandType = _sessionManager.GetDDListLandType();
                ViewBag.LandType = AllLandType;
                return View(Constants.LandresultViewPath + Constants.ViewRecord, landInspectionList);
        ...
        public IActionResult Edit(long InspectionId, int row,string id = "")
        {
            try
            {
                ...
                _logger.Information(CommonLogHelpers.ViewRecordCompleted + id);
                return View(Constants.LandresultViewPath + Constants.editCs, landInspectionList);
        ...
        public IActionResult Edit(string id, IdmDchgLandDetDTO landInspection)
        {
            try
            {
                ...
                ViewBag.InspectionId = landInspection.DcLndIno;
                return Json(new { isValid = false, html = Helper.RenderRazorViewToString(this, Constants.LandviewPath + Constants.Edit, landInspection) });
            }
            catch (System.Exception ex)
            {
        ...
        public IActionResult Delete(long InspectionId,string id)
        {
            try
            {
                ...
                {
                    activeLand = landInspectionList.Where(x => x.IsActive == true && x.IsDeleted == false).ToList();
                }
                ...
                return Json(new { isValid = true, html = Helper.RenderRazorViewToString(this, Constants.LandviewPath + Constants.ViewAll, activeLand) });
