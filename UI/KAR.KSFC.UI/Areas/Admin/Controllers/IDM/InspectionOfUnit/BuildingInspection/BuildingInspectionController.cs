...
namespace KAR.KSFC.UI.Areas.Admin.Controllers.IDM.InspectionOfUnit.BuildingInspection
{
    [Area(Constants.Admin)]
    [Authorize(Roles = RolesEnum.Employee)]

    public class BuildingInspectionController : Controller
    {
        private readonly ILogger _logger;
        private readonly SessionManager _sessionManager;
       

        public BuildingInspectionController( ILogger logger, SessionManager sessionManager)
        {
        ...
        public IActionResult Create(long accountNumber,long InspectionId,byte OffcCd,string LoanSub, bool firstinspection)
        {
            try
            {
               var buildingDetails = _sessionManager.GetAllBuildingInspectionList();
                ...
                ViewBag.firstbuildinspection = firstinspection;
                ...
                //if (buildingInspectionDetails.Count > 0 || isfirstinspection == InspectionId)
                //{
                //    ViewBag.firstbuildinspection = false;
                //}
                if (buildingInspectionDetails.Where(x => x.DcBdgIno == InspectionId).Count() > 0 || forfirstinspection == InspectionId)
                {
                    ViewBag.firstbuildinspection = false;
                }
                ...
                return View(Constants.BuildingInspectionresultViewPath + Constants.createCS);
            }
        ...
        public IActionResult Create(IdmDchgBuildingDetDTO model)
        {
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    return Json(new { isValid = true, data = list.LoanAcc, html = Helper.RenderRazorViewToString(this, Constants.BuildingInspectionviewPath + Constants.ViewAll, activeList) });
                }
                ViewBag.AccountNumber = model.LoanAcc;
                ViewBag.InspectionId = model.DcBdgIno;
                ViewBag.LoanSub = model.LoanSub;
                ViewBag.OffcCd = model.OffcCd;
                return Json(new { isValid = false, html = Helper.RenderRazorViewToString(this, Constants.BuildingInspectionviewPath + Constants.Create, model) });
            }
            catch (System.Exception ex)
            {
        ...
        public IActionResult ViewRecord(string id = "")
        {
            try
            {
                ...
                return View(Constants.BuildingInspectionresultViewPath + Constants.ViewRecord, buildingInspectionList);
               ...
               if(buildingInspectionList!= null)
                {
                    ...
                    //ViewBag.InspectionId = buildingInspectionList.DcBdgIno;
                ...
                _logger.Information(CommonLogHelpers.ViewRecordCompleted + id);
                return View(Constants.BuildingInspectionresultViewPath + Constants.editCs, buildingInspectionList);
        ...
        public IActionResult Edit(string id, IdmDchgBuildingDetDTO inspection)
        {
            try
            {
                ...
                if (buildingInspectionExist != null)
                {
                    ...
                    _sessionManager.SetBuildingInspectionList(buildingInspectionDetails);
                    List<IdmDchgBuildingDetDTO> activeList = buildingInspectionDetails.Where(x => x.IsActive == true && x.IsDeleted == false).ToList();
                    return Json(new { isValid = true, data=list.LoanAcc,html = Helper.RenderRazorViewToString(this, Constants.BuildingInspectionviewPath + Constants.ViewAll, buildingInspectionDetails) });
                }
                ...
                return Json(new { isValid = false, html = Helper.RenderRazorViewToString(this, Constants.BuildingInspectionviewPath + Constants.Edit, inspection) });
            }
            catch (System.Exception ex)
            {
        ...
        public IActionResult Delete(string  id)
        {
            try
            {
                ...
                ViewBag.firstbuildinspection = true;
                ViewBag.inspectiondetails = inspectiondetailslist;
                string html = Helper.RenderRazorViewToString(this, Constants.BuildingInspectionviewPath + Constants.ViewAll, activeBuildingInsp);
