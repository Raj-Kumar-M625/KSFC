...
namespace KAR.KSFC.UI.Areas.Admin.Controllers.IDM.InspectionOfUnit.FurnitureInspection
{
    [Area(Constants.Admin)]
    [Authorize(Roles = RolesEnum.Employee)]

    public class FurnitureInspectionController : Controller
    {
        private readonly IInspectionOfUnitService _inspectionOfUnitService;
        private readonly ILogger _logger;
        private readonly SessionManager _sessionManager;
      
        public FurnitureInspectionController(IInspectionOfUnitService inspectionOfUnitService, ILogger logger, SessionManager sessionManager)
        {
        ...
        //Author Sandeep M on 30/08/2022
        [HttpGet]
        public IActionResult Create(long accountNumber, long InspectionId, byte OffcCd, string LoanSub, bool firstinspection)
        {
            ...
            try
            {
                ...
                ViewBag.firstfurnitureinspection = firstinspection;


                //if(furnitureDetails.Count > 0 || isfirstinspection == InspectionId)
                //{
                ...
                if (furnitureDetails.Where(x => x.FurnIno == InspectionId).Count() > 0 || forfirstinspection == InspectionId)
                {
                    ViewBag.firstfurnitureinspection = false;
                }
                ...
                return View(Constants.FurnitureresultViewPath + Constants.createCS);
            }
        ...
        public IActionResult Create(IdmDChgFurnDTO furnitureInspection)
        {
            ...
            try
            {
                ...
                return Json(new { isValid = true, data = list.LoanAcc, html = Helper.RenderRazorViewToString(this, Constants.FurnitureviewPath + Constants.ViewAll, activeList) });
        ...
        public IActionResult ViewRecord(string id = "")
        {
            try
            {
                ...
                return View(Constants.FurnitureresultViewPath + Constants.ViewRecord, FurnitureInspectionList);
        ...
        public IActionResult Edit(long InspectionId,int row, string id = "")
        {
            try
            {
                ...
                _logger.Information(CommonLogHelpers.ViewRecordCompleted + id);
                return View(Constants.FurnitureresultViewPath + Constants.editCs, furnitureInspectionList);
        ...
        public IActionResult Edit(string id, IdmDChgFurnDTO furnitureInspection)
        {
            try
            {
                ...
                return Json(new { isValid = false, html = Helper.RenderRazorViewToString(this, Constants.FurnitureviewPath + Constants.Edit, furnitureInspection) });
            }
            catch (System.Exception ex)
            {
        ...
        public IActionResult Delete(string id)
        {
            try
            {
                ...
                return Json(new { isValid = true, html = Helper.RenderRazorViewToString(this, Constants.FurnitureviewPath + Constants.ViewAll, activeFurn) });
        ...
        [HttpPost]
        public async Task<IActionResult> SaveFurnitureInspectionDetails()
        {
            try
            {
                if (_sessionManager.GetAllFurnitureInspectionList().Count != 0)
                {

                    _logger.Information(CommonLogHelpers.StratedSaveLandInspectionDetails);

                    var FurnitureInspectionList = _sessionManager.GetAllFurnitureInspectionList();

                    foreach (var item in FurnitureInspectionList)
                    {
                        switch (item.Action)
                        {
                            case (int)Constant.Delete:
                                await _inspectionOfUnitService.DeleteFurnitureInspectionDetails(item);
                                _logger.Information(CommonLogHelpers.CompletedFurnitureInspectionDelete);
                                break;
                            case (int)Constant.Update:
                                await _inspectionOfUnitService.UpdateFurnitureInspectionDetails(item);
                                _logger.Information(CommonLogHelpers.CompletedFurnitureInspectionDelete);
                                break;
                           case (int)Constant.Create:
                                await _inspectionOfUnitService.CreateFurnitureInspectionDetails(item);
                                    _logger.Information(CommonLogHelpers.CompletedFurnitureInspectionCreate);
                                break;
                            default:
                                break;
                        }
            ...
            {
                _logger.Error(Error.SaveFurnitureInspection + ex.Message + Environment.NewLine + Error.Stack + ex.StackTrace);
