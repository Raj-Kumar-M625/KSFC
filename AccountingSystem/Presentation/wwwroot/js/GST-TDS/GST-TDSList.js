...
function setParams() {
    ...
    var status = $('#filtergsttdsFilters_gstTdsStatus').val();
    console.log(status);
    Id = $('#GSTTDSListGrid [name=Id]').filter(function () {
    ...
    if (Id.length == 0) {
        Swal.fire({
            text: 'Please select atleast one  bill!!',
            icon: 'warning',
        ...
        Swal.fire({
            text: 'Please Select Pending Bills !!',
            icon: 'warning',
            confirmButtonText: 'Ok',
        })
        $('.awe-popup').each(function () {
...
function outsideFilter(o) {
    ...
    g.on('awefinit', function () {
        ...
        g.on('awerender', function (e) {
            ...
            $.when($.post(`/GSTTDS/GetTotalGSTTDSPayment`))
                .done(function (res) {
   ...
   // var grid = $('#@(gridId)');
    ...
    function processSelections() {
        ...
        if (Object.keys(vals).length == 0) {
            ...
            var sum = 0;
            $.each(selItems, function (i, item) { sum += parseFloat(item.gstTdsPayableAmount); });
...
function exportToExcel() {
    ...
    var url = `/${contr}/ExportGstTdsList`;
    //var url = '@Url.Action("ExportGstTdsList", contr)';
...
function dayfirst(opt) {
    //var toDate = $('#filtergsttdsFilters_paymentToDate').val();
    //site.dayfirst(opt, toDate);
}

function daylast(opt) {
    // var fromDate = $('#filtergsttdsFilters_paymentFromDate').val();
